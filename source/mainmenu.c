...
void mainmenu(bool top) {
	...
	if (kDown & KEY_A) {
		switch (selectedMenuOption) {
			case 0:
				transitionState(GAME_STATE_IN_GAME, TRANSITION_TYPE_SWIPE_UP, seconds(1), EASE_IN_EXPO);
				break;
			case 1:
	...
	if (transition_timer != 0) {
		if ((previous_state == GAME_STATE_OPTIONS && !transition_forwards) || (after_transition == GAME_STATE_OPTIONS && transition_forwards)) {
			xoffset = (TOP_SCREEN_WIDTH / 10) * lerp((transition_timer / (float)transition_duration), EASE_IN_EXPO);
		}
	}

	draw_text("Lucky Rollers", TOP_SCREEN_WIDTH / 2 - xoffset, 24);

	float offset = (TOP_SCREEN_HEIGHT - 64 - 48) / (mainMenuOptions - 1);
	for (int i = 0; i < mainMenuOptions; i++) {
		draw_text_colour(mainmenuitems[i], TOP_SCREEN_WIDTH / 2 - xoffset, 64 + offset * i, i == selectedMenuOption ? C2D_Color32(0x38, 0xa3, 0xa5, 0xff) : C2D_Color32(0x9d, 0x02, 0x08, 0xff));
	}
}